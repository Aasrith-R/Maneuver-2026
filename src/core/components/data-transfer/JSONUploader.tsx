...
import { downloadCsv, jsonToCsv, type JsonDataType } from "@/core/lib/data-transfer/jsonToCsv";
...
const JSONUploader: React.FC<JSONUploaderProps> = ({ onBack }) => {
  ...
  const handleConvertToCsv = async (): Promise<void> => {
    if (!selectedFile || !detectedDataType) {
      toast.error("Please select a file first");
      return;
    }

    if (detectedDataType === "pitScoutingImagesOnly") {
      toast.error("Images-only data cannot be exported as CSV");
      return;
    }
    ...
    try {
      const text = await selectedFile.text();
      const jsonData: unknown = JSON.parse(text);
      const csv = jsonToCsv(detectedDataType as JsonDataType, jsonData);
      ...
      downloadCsv(csv, `${baseName}-${safeTimestamp}.csv`);
              ...
              <>
                <div className="space-y-2">
                  <Button
                    ...
                    disabled={!detectedDataType || detectedDataType === 'pitScoutingImagesOnly' || isConverting}
                    variant="secondary"
                    className="w-full min-h-16 text-xl whitespace-normal text-wrap py-3 px-4 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {detectedDataType === 'pitScoutingImagesOnly'
                      ? 'Images Cannot Be Downloaded as CSV'
                      : isConverting
                        ? 'Converting...'
                        : `Download ${detectedDataType === 'scouting' ? 'Scouting Data' : detectedDataType === 'scoutProfiles' ? 'Scout Profiles' : 'Pit Scouting Data'} as CSV (No Import)`
                    }
                  </Button>
                  <p className="text-xs text-muted-foreground text-center">
                    Converts the selected JSON file directly to CSV without modifying local data.
                  </p>
                </div>
                <Separator />
                {detectedDataType === 'pitScoutingImagesOnly' ? (
